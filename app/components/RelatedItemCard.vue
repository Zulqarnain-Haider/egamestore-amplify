...
const props = defineProps({
  ...
  price: { type: [String, Number], default: '1249.99' },
  rating: { type: [String, Number], default: 80 },
})

// product computed ko props se banao
const product = computed(() => ({
  id: props.title ? 'related-' + props.title.replace(/\s+/g, '-').toLowerCase() : 'related-default',
  image: props.image,
  title: props.title,
  price: Number(props.price),
  rating: Number(props.rating),
  platforms: ['PC', 'Xbox', 'PlayStation'], // Static, ya props se add karo agar chaho
  regions: ['Global'],
}))
